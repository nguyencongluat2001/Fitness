...
<div id="style-1" style="padding-right:10px;">
  <div class="table-responsive pmd-card pmd-z-depth table-container">
    <table id="table-data" class="table  table-bordered table-striped table-condensed dataTable no-footer @if(!Auth::check()) onload @endif" @if(!isset($_SESSION['id']) && (!empty($_SESSION['account_type_vip']) && $_SESSION['account_type_vip'] != 'VIP1')) onclick="JS_Signal.checkLogin()" @endif>
      <colgroup>
        <col width="3%">
        <col width="5%"> <!-- macp -->
        <col width="6%"> <!-- san -->
      ...
      <tbody id="body_data" style="background:#dbead3;" @if(!Auth::check()) class="blur" @endif>
        @if(Auth::check() && (!empty($_SESSION['account_type_vip']) && $_SESSION['account_type_vip'] == 'VIP1'))
