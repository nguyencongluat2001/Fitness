...
JS_UserLog.prototype.loadevent = function(oForm) {
        var myClass = this;
        $('form#frmAdd').find('#btn_create').click(function() {
            myClass.store('form#frmAdd');
        })
 ...
 * @return void
 */
JS_UserLog.prototype.add = function(oForm) {
 ...
 * @return void
 */
JS_UserLog.prototype.store = function(oFormCreate) {
 ...
 * @return void
 */
JS_UserLog.prototype.loadList = function(oForm, numberPage = 1, perPage = 15) {
        var myClass = this;
 ...
 * @return void
 */
JS_UserLog.prototype.edit = function(id) {
    var url = this.urlPath + '/edit';
    var myClass = this;
    var data = '_token=' + $('#frmUserLog_index #_token').val();
    data += '&id=' + id;
    $.ajax({
        url: url,
        type: "GET",
...
// Xoa mot doi tuong
JS_UserLog.prototype.delete = function(oForm) {
        var myClass = this;
        var listitem = '';
 ...
 */
JS_UserLog.prototype.addrow = function() {
        ...
        html += '<span class="custom-control-indicator p-0 m-0" onclick="JS_UserLog.changeStatusUserLog(\'' + id + '\')"></span>';
        html += '</label></td>';
        // edit
        html += '<td align="center"><span class="text-cursor text-warning" onclick="JS_UserLog.edit(\'' + id + '\')"><i class="fas fa-edit"></i></span></td>';
        html += '</tr>';
...
function click2(id, type) {
    ...
    $("#" + type + "_" + id).focusout(function() {
        ...
        if (text != $(".span_" + type + '_' + id).html()) {
            JS_UserLog.updateUserLog(id, type, $(".span_" + type + '_' + id).html());
        }
    })
}
/**
 ...
 */
JS_UserLog.prototype.updateUserLog = function(id, column, value = '') {
        var myClass = this;
        var url = myClass.urlPath + '/updateUserLog';
        var data = 'id=' + id;
        data += '&_token=' + $('#frmUserLog_index').find('#_token').val();
 ...
 * Thay đổi trạng thái
 */
JS_UserLog.prototype.changeStatusUserLog = function(id) {
    var myClass = this;
    var url = myClass.urlPath + '/changeStatusUserLog';
    var data = '_token=' + $("#frmUserLog_index #_token").val();
 ...
 * Check
 */
JS_UserLog.prototype.checkValidate = function(){
    if($("#title").val() == ''){
